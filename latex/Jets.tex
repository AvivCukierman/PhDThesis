...
Other massive particles, like $W/Z$ bosons, the Higgs boson, top quarks, and $\tau$ particles, are also produced and play vital roles in searches and measurements at the LHC; the hadronic decays of these particles either are the primary way or an extremely important way of identifying their production in an event.
...
Small-$R$ jets are intended to approximate light particles, in particular quarks and gluons, though in ATLAS $\tau$ particles, photons, and electrons are in addition reconstructed as small-$R$ jets in the calorimeter; large-$R$ jets are intended to approximate boosted massive objects decaying hadronically.
...
Photons and electrons decay primarily in the electromagnetic calorimeter, but as the seeds are formed from clusters in the electromagnetic and hadronic calorimeters, and the typical size of photon and electron decays is $\sim 0.2$~\cite{ref:TODO}, these are also reconstructed as small-$R$ jets.
...
$\tau$ particles decay hadronically roughly 65\% of the time~\cite{ref:TODO}, although with their mass around 1.7 \GeV{} they are reconstructed as small-$R$ jets.
...
If there are $N$ seeds in the event, then the set of \kt{} algorithms require calculating the pair-wise distance $d_{ij}$ among all proto-jets, which takes $O(N^2)$ time; and then repeating that process until all jets have been found, which in general is $O(N)$.
...
In addition, as will be mentioned in~\ref{sec:LHC:luminosity}, the collisions occur in whole bunches of $10^{11}$ protons, so that there are multiple simultaneous interactions in single bunch crossing; this is referred to as \textit{in-time pile-up}.
...
There have been a variety of grooming techniques proposed for large-$R$ jets~\cite{Aad:2013gja}, but the standard contemporary technique in ATLAS is jet trimming~\cite{Krohn:2009th}.
