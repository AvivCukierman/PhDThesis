...
\begin{figure}[htbp]
  ...
  \label{fig:ATLAS:xs}
\end{figure}
This paucity of ``interesting'' physics motivates the choice to set up multiple (in 2018, about $60$~\cite{luminositypublic}) proton-proton interactions per bunch crossing, or pile-up - in order to increase the probability of a such an interaction to occur in any given bunch crossing.
...
L1Calo sets up $0.1\times0.1$ in $\eta\times\phi$ trigger towers in the calorimeter and forms \textit{regions of interest}, or \textit{RoIs}, to identify physics objects.
