...
This search is notably limited to $m_a>20$ \GeV{}; the ATLAS Run 1 search for $H\rightarrow aa \to 4\gamma$~\cite{Aad:2015bua} similarly sets limits only for $m_a>10$ \GeV{}.
These lower bounds are due to the limitations set by using the diphoton trigger (Section~\ref{sec:HBSM:trigger}).
For low $m_a$, the $a$ particle is boosted and the decay products are collimated - this ruins the diphoton trigger efficiency, as isolation is imposed on the photons at various stages in the trigger and offline object reconstruction.
...
This search presents many interesting areas of research in order to improve the sensitivity of the analysis for the resolved mass range.
...
Each of these challenges would be better tackled by improving the energy and \pt{} resolution of the jets involved, since the invariant mass is a simple function of the individual jet energies and the angle between them\footnote{The VBF selection also relies on the resolution of the VBF $m_{jj}$.}.
