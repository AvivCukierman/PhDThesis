...
We have found (Fig.~\ref{fig:HBSM:HBSM_prodmodes}) that the search in the VBF production 
...
Fig.~\ref{fig:HBSM:VBFH_feynman} shows a tree-level Feynman diagram of the VBF production and $\gamma\gamma gg$ decay of the Higgs boson, including the $aa$ intermediate state.
...
The $m_{\gamma\gamma}$ resolution is excellent, with $>\sim97\%$ of signal events satisfying $|m_{\gamma\gamma}-m_a|<2.5$ \GeV{}, regardless of the value of $m_a$; this can be seen in Fig.~\ref{fig:HBSM:photon_kinematics}.
...
Central jets with $p_T<60$ GeV and $|\eta|$<2.4 are required to satisfy the \textit{Default} Jet Vertex Tagger selection~\cite{PERF-2014-03}.
...
Since the efficiency of the trigger and preselections is very low on signal with $m_a<20$ GeV (as can be seen in Fig.~\ref{fig:HBSM:cutflow_ma}), the lowest $m_{\gamma\gamma}$ regime starts at $17.5$ GeV.
...
The efficiency of the various analysis cuts in the signal can be seen in Fig.~\ref{fig:HBSM:cutflow_ma}.
...
The distribution of events across $|m_{jj}-m_{\gamma\gamma}|$ (i.e., the second axis of the A/B/C/D method), separated into Tight--Tight and Tight--Loose events (i.e., the first axis of the A/B/C/D method) can be seen in Fig.~\ref{fig:HBSM:amassdiff}, for both the validation and signal regions.
...
Fig.~\ref{fig:HBSM:ABCD_bars} shows the distribution of the data in each of the $m_{\gamma\gamma}$ analyses regimes, in the validation and signal regions, with an expected signal that has high efficiency in that analysis regime.
...
This information is also presented in Fig.~\ref{fig:HBSM:ABCD_bars_unblinded}.
\begin{figure}[htbp]
  \centering 
...
The prior distribution of the number of events observed in the signal $D$ region is shown in Fig.~\ref{fig:HBSM:NinD_dist}.
