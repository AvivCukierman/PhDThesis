...
As the first demonstation of this technique, the neural network uses a reduced set of features of just the masses of the two jets to tag potential signals.
...
\footnote{In fact, the incusive dijet search uses small-$R$ jets, so that the decay products of the $B$ and $C$ are not necessarily entirely contained within the jet. Because of this, as the masses of the $B$ and $C$ particle get larger for fixed $m_A$, the limits from the incusive dijet search get worse.}.
Also relevant are the results from the ATLAS all-hadronic diboson resonance search~\cite{Aad:2019fbh}, which targets signal models of the form $A\rightarrow WW/WZ/ZZ$ and is thus sensitive to signals with $m_B~m_C~m_{W/Z}$
...
However, those limits are order of magnitude weaker than the ones studies here and are therefore not considered further.
...
In a standard search for BSM at the LHC, some particular signal model is designated and an event selection is optimized to target this signal relative to the expected background using signal simulation some some proxy for the background (either simulation- or data-based).
...
\footnote{
Or rather, if there are differences between background events in Sample 1 and background events in Sample 2, those differences must be smaller than the difference in signal/background ratio times the difference between signal and background between the two samples.  
}.
...
\caption{
  ...
  Figure adapted from~\ref{Metodiev:2017vrx}.
}
\label{fig:CWoLa:cwolasetup}
\end{figure}

It is worth noting the properties and limitations of this method.
First, the features used to distinguish between signal and background events cannot or can only be very bad at distinguishing between background events in Mixed Sample 1 and Mixed Sample 2 (and similarly for signal events, though in this analysis the features are chosen such that the signal events have localized distributions in those features and therefore this is not as much of a concern).
...
The idea of ``CWoLa hunting''~\cite{Collins:2018epr,Collins:2019jip} is to use the CWoLa method to be search for BSM physics without a particular signal model in mind.
...
Because the two mixed samples come from nearby regions of $m_\text{res}$, the difference in distributions of features in the background between the two mixed samples can be smaller than the fraction of signal times the difference in distributions between the signal and background.
...
This and other key aspects are discussed in the following Section, which lays out the assumptions and details of this analysis based on the above ideas with $m_\text{res}=m_{JJ}$.
...
%-------------------------------------------------------------------------------
Computing resources:~\ref{app:CWoLa:computing}.
